...
export default function StudioPreviewFrame({ children, name, chatOpen, onChatChange, onNew, onExample, dark, onAppearance }: {
  ...
  const chat = <Stack sx={{ height: '100%', p: 2 }} spacing={2}>
    <Stack direction="row" sx={{  justifyContent: "space-between", alignItems: "center" }}><Typography variant="h6">AI Chat</Typography><Button onClick={() => onChatChange(false)}>Hide</Button></Stack>
  ...
  return <Box sx={{ height: '100dvh', display: 'flex', flexDirection: 'column', bgcolor: 'background.default' }}>
    <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", px: 2, py: 1, borderBottom: 1, borderColor: 'divider' }}><Typography sx={{  fontWeight: 600 }}>AI Curation · Agent Studio</Typography><Button onClick={onAppearance}>{dark ? 'Light' : 'Dark'} appearance</Button></Stack>
    <Root sx={{ minHeight: 0 }}>
