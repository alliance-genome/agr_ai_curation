...
export default function DirectExportSetting({ value, onChange, isDefault = false, readOnly = false }: {
  value: 'ai' | 'direct'
  ...
  return <Box sx={{ display: 'grid', gap: 1 }}>
    <Box sx={{ display: 'flex', alignItems: 'center' }}>
      <FormControlLabel sx={{ m: 0, gap: 0.75, '& .MuiFormControlLabel-label': { typography: 'body2' } }}
        control={<Switch size="small" disabled={readOnly} slotProps={{ input: { role: 'switch' } }} checked={value === 'direct'}
