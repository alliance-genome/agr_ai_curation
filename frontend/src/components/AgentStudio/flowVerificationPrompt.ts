...
2. Reconstruct exact instructions with get_current_flow_instructions(node_id, field, cursor, limit): task_instructions, every present custom_instructions, and each step_goal relevant to a judgment. Follow next_cursor until complete=true for every required field.
3. Inspect get_current_flow_topology sections issues, control_path, control_edges, output_bindings, and validation_sidecars. Fetch relevant scalar node details, output projection-plan fields or JSON-Pointer sections, validation-warning pages, and validation-schedule sections (selections, scheduled_validators, opt_outs, replacement_validators, supplemental_validators, inactive_metadata) only when the verification criteria require them. Follow every cursor/completion signal.
...
6. For document/PDF capability claims, use get_tool_inventory(agent_id=<node agent>) or another focused query, then method/PDF-level get_tool_details(tool_id, agent_id). Do not use an unsafe global inventory or oversized parent-tool metadata.
