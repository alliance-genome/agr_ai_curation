...
export default function OutputStructureWorkflow(props: OutputStructureEditorProps) {
  ...
  return <ProfileValidatorCatalog value={value}><Stack spacing={3}>
    ...
    {stage === 'review' && <Stack spacing={3}>
      ...
      <Stack direction="row" sx={{  gap: 2, flexWrap: "wrap" }}><Button onClick={() => setStage('edit')}>Back to details</Button><Button variant="contained" disabled={disabled || validating || issues.length > 0} onClick={() => setStage('done')}>Finish {value.name}</Button></Stack>
    </Stack>}
    {stage === 'done' && <Stack spacing={3}>
      <Typography component="h2" variant="h5">Your extraction plan</Typography>
      <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap", py: 3, borderTop: 1, borderBottom: 1, borderColor: 'divider' }}>
