    ...
    {stage === 'review' && <Stack spacing={3}>
      ...
      <TableContainer sx={{ position: 'relative', border: 1, borderColor: 'divider', borderRadius: 1 }} tabIndex={0} role="region" aria-label="Review table; scroll horizontally on small screens">
        <Table aria-label="Extraction plan" sx={{ minWidth: 440 }}>
          <TableHead sx={{ bgcolor: 'action.hover' }}><TableRow><TableCell>Detail or part</TableCell><TableCell>Always include</TableCell><TableCell>Empty answer allowed</TableCell></TableRow></TableHead>
          <TableBody>{rows.map((row) => <TableRow key={row.schemaPath} sx={{ bgcolor: row.depth ? 'action.hover' : undefined }}>
            ...
            <TableCell>{row.field.nullable ? 'Yes' : 'No'}</TableCell>
          </TableRow>)}
          {rows.length === 0 && <TableRow><TableCell colSpan={3} sx={{ py: 4, textAlign: 'center' }}>No details yet. Go back to add the information you want to collect.</TableCell></TableRow>}
          </TableBody>
        </Table>
      </TableContainer>
