...
function ExampleValue({ schema }: { schema: GenericProfileValueSchema }) {
  ...
  if (schema.kind === 'object') return <Box component="dl" sx={{ m: 0 }}>
    {schema.fields.map((field, index) => <Box key={index} sx={{ mb: 1 }}>
      <Typography component="dt" variant="body2" sx={{  fontWeight: 600 }}>{field.display_name || field.key || 'Unnamed field'}</Typography>
...
function ValueSchemaEditor({ schema, onChange, onChangeKind, onBlur, path, issueText, disabled, isPart }: {
  ...
  return <Stack spacing={2}>
    <>
      <Typography id={`profile-${answerPath}`} tabIndex={-1} sx={{  fontWeight: 600 }}>What should one answer look like?</Typography>
...
export default function OutputStructureEditor({ value, onChange, onValidate, issues, validating = false, disabled = false, onAskAI }: OutputStructureEditorProps) {
  ...
  const addDetailForm = adding !== null ? <Box component="form" onSubmit={(event) => { event.preventDefault(); createField() }} sx={{ my: 2, p: 2, bgcolor: 'action.hover' }}>
    <Typography sx={{  fontWeight: 600 }}>{adding.length ? `Add a part to ${rows.find((entry) => entry.address.join('.') === adding.join('.'))?.field.display_name || 'this answer'}` : 'Add a detail'}</Typography>
    ...
    <Stack direction="row" sx={{ gap: 1, mt: 1 }}><Button type="submit" disabled={disabled || !newName.trim()}>Add detail</Button><Button onClick={() => setAdding(null)}>Cancel</Button></Stack>
  </Box> : null
  if (!value.name.trim()) return <Stack spacing={3} sx={{ maxWidth: 720, py: 3 }}>
    <Box><Typography component="h2" variant="h5" sx={{  fontWeight: 600 }}>What do you want to extract?</Typography>
  ...
  return <Stack spacing={3} className="collection-builder" sx={(theme) => ({
    ...
    <Box>
      <Typography component="h2" variant="h5" sx={{  fontWeight: 600 }}>{value.name}</Typography>
    ...
    {onAskAI && <Stack className="collection-ai" direction={{ xs: 'column', sm: 'row' }} sx={{  justifyContent: "space-between", alignItems: { sm: 'center' }, gap: 2 }}>
      <Box><Typography sx={{  fontWeight: 600 }}>Tell AI Chat what you need</Typography><Typography color="text.secondary">Describe the details you need, and AI Chat can help build them.</Typography></Box>
    ...
    {issues.length > 0 && <Alert severity="error"><Typography sx={{  fontWeight: 600 }}>Some settings need attention. Your draft is preserved.</Typography>
    ...
    <Box role="tabpanel" hidden={view !== 'example'} id="collection-panel-example" aria-labelledby="collection-tab-example">
      ...
      <Box component="dl" sx={{ m: 0 }}>{value.fields.map((item) => <Box className="collection-example-row" key={item.key}>
        <Typography component="dt" sx={{  fontWeight: 600 }}>{item.display_name || item.key}</Typography>
        <Box component="dd" sx={{ m: 0 }}><ExampleValue schema={item.value_schema} /></Box>
      </Box>)}</Box>
    </Box>
    <Stack direction={{ xs: 'column', sm: 'row' }} className="collection-footer" sx={{  gap: 2, justifyContent: "space-between" }}>
      <Typography color="text.secondary">Edits stay in your draft until you save the agent.</Typography>
      <Stack direction="row" sx={{  gap: 1 }}><Button onClick={() => setChecks(true)}>All validator settings{value.validator_mappings?.length ? ` (${value.validator_mappings.length})` : ''}</Button><Button disabled={disabled || validating} onClick={onValidate}>Validate fields</Button></Stack>
    </Stack>
    </Stack>
    <Popover open={Boolean(help)} anchorEl={help?.anchor} onClose={() => setHelp(null)} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}>
      <Box sx={{ p: 3, maxWidth: 380 }}><Typography sx={{  fontWeight: 600 }}>{help?.title}</Typography><Typography sx={{ mt: 1, whiteSpace: "pre-line" }}>{help?.text}</Typography><Button onClick={() => setHelp(null)} sx={{ mt: 1 }}>Close</Button></Box>
    </Popover>
    {row && field && <Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'minmax(150px, 1fr) minmax(0, 3fr)', gap: 3, '@container workshop (max-width: 680px)': { gridTemplateColumns: 'minmax(0, 1fr)' } }}>
        <Box component="nav" aria-label="Collection details" sx={{ borderRight: 1, borderColor: 'divider', pr: 2 }}>
          <Typography sx={{ fontWeight: 600, mb: 1 }}>Details in {value.name}</Typography>
        ...
        <Box component="section" aria-label={`Edit ${field.display_name || field.key}`}>
          <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap", mb: 3 }}><Typography component="h3" variant="h6">{field.display_name || field.key}</Typography><Stack direction="row" sx={{  gap: 1, flexWrap: "wrap" }}><Button onClick={closeEditor}>Back to all details</Button><Button variant="contained" onClick={finishDetail}>{doneLabel}</Button></Stack></Stack>
          ...
          <TableContainer sx={{ position: 'relative', border: 1, borderColor: 'divider', borderRadius: 1 }} tabIndex={0} role="region" aria-label="Parts table; scroll horizontally on small screens">
            <Table aria-label={`Parts of ${field.display_name || field.key}`} sx={{ minWidth: 460 }}>
              <TableHead sx={{ bgcolor: 'action.hover' }}><TableRow>
                ...
                <TableCell sx={{ fontWeight: 700 }}><Stack direction="row" sx={{  alignItems: "center", gap: 0.5 }}>
              ...
              <TableBody>
                {childSchema(field.value_schema)!.fields.map((part, index) => {
                  ...
                  return <TableRow key={part.key}>
                    ...
                    <TableCell><Checkbox slotProps={{ input: { 'aria-label': `Always include ${name} with this answer` } }} disabled={disabled} checked={part.required ?? false} onChange={(_, required) => onChange(updateProfileField(value, address, { required }))} /></TableCell>
                ...
                {childSchema(field.value_schema)!.fields.length === 0 && <TableRow><TableCell colSpan={5} sx={{ py: 4, textAlign: 'center' }}>
                  <Typography sx={{  fontWeight: 600 }}>No parts yet</Typography>
        ...
        <Divider />
        <Box><Typography sx={{  fontWeight: 600 }}>When should this detail be included?</Typography>
        ...
        <Divider />
        <Box><Stack direction="row" sx={{  justifyContent: "space-between", alignItems: "center" }}><Typography sx={{  fontWeight: 600 }}>Instructions for this detail</Typography>
        ...
        {issues.filter((issue) => issue.path.startsWith(row.schemaPath)).map((issue, index) => <Alert severity="error" key={index}>{issue.message}</Alert>)}
          ...
          <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", gap: 2, flexWrap: "wrap", mt: 3, pt: 2, borderTop: 1, borderColor: 'divider' }}><Typography color="text.secondary" variant="body2">Your changes are kept in this draft.</Typography><Button variant="contained" onClick={finishDetail}>{doneLabel}</Button></Stack>
        </Box>
      </Box>
    </Box>}
