...
export default function ProfileValidatorEditor({ value, onChange, onValidate, issues, disabled = false }: Props) {
  ...
  const load = async (after?: string) => {
    ...
    try {
      ...
      const result = await getProfileMappingOptions({ ...value, validator_mappings: [] }, after)
  ...
  const add = (cap: ProfileValidatorOptions, slot: string) => {
    ...
    const field = data?.fields.find((field) => field.path === selectedPath)
    ...
    onChange({ ...value, validator_mappings: [...mappings, {
      ...
      inputs: { [slot]: { source: 'field', field_path: selectedPath } }, outputs: {},
  ...
  return <Stack component="section" aria-label="Optional semantic validators" spacing={1.5}>
    <Typography variant="h6" component="h3">Optional semantic validators</Typography>
    <Typography>Structure checks always enforce the saved fields and value types. Semantic validators add explicitly mapped checks; they do not make this a LinkML object or establish submission readiness.</Typography>
    {mappings.length === 0 && <Alert severity="info">No semantic validators mapped. Structural validation remains enforced.</Alert>}
    <Button disabled={disabled || loading} onClick={() => void load()}>{options ? 'Refresh compatible validators' : 'Find compatible validators'}</Button>
    ...
    {mappings.map((mapping, index) => {
      ...
      return <Stack component="fieldset" id={`profile-${prefix}`} tabIndex={-1} disabled={disabled} key={mapping.mapping_id} spacing={1.5} sx={{ minWidth: 0, borderColor: 'divider' }}>
        <Typography component="legend">{cap?.metadata.display_name || mapping.capability_ref.binding_id} · {mapping.mapping_id}</Typography>
        <Typography variant="body2">Package {mapping.capability_ref.package_id} {mapping.capability_ref.package_version}; binding {mapping.capability_ref.binding_id}. {cap ? (cap.state === 'under_development' ? 'Under development' : 'Active') : 'Capability details not loaded.'}</Typography>
        <Stack spacing={0.5} sx={{ overflowWrap: 'anywhere' }}>
          {Object.entries(mapping.inputs).map(([slot, source]) => <Typography key={slot} variant="body2">Input {slot}: {source.source === 'context' ? 'package-owned context' : source.source === 'constant' ? `fixed value — ${constantSummary(source.value)}` : source.field_path || 'field not selected'}</Typography>)}
        ...
        {reuse && <>
          <TextField select label={`Mapping mode · ${mapping.mapping_id}`} value={mapping.mode ?? 'whole'} disabled={disabled} onBlur={onValidate}
          ...
          {Object.entries(reuse.inputs).map(([slot, input]) => {
            ...
            const label = `${slot} · ${mapping.mapping_id}`
            ...
            return <Stack key={slot} spacing={1}>
              <TextField select label={`Input source · ${label}`} value={source?.source ?? (source ? 'field' : '')} disabled={disabled} onBlur={onValidate}
                ...
                {input.allow_field && <MenuItem value="field">Canonical profile field</MenuItem>}
                {input.allow_constant && <MenuItem value="constant">Fixed typed value</MenuItem>}
                {input.context_selector && <MenuItem value="context">Package-owned record context</MenuItem>}
              </TextField>
              {source && (source.source ?? 'field') === 'field' && <TextField select label={`Input field · ${label}`} value={source.field_path ?? ''} disabled={disabled} onBlur={onValidate}
                ...
                {choices.map((path) => <MenuItem key={path} value={path}>{path}</MenuItem>)}
              </TextField>}
          ...
          {Object.entries(reuse.outputs).map(([slot, output]) => {
            ...
            return <TextField select key={slot} label={`Write ${slot} to · ${mapping.mapping_id}`} value={destination} disabled={disabled} onBlur={onValidate}
          ...
          <TextField select label={`If unresolved · ${mapping.mapping_id}`} value={mapping.policy.unresolved} disabled={disabled} onBlur={onValidate}
          ...
          <TextField select label={`Readiness and export · ${mapping.mapping_id}`} value={String(mapping.policy.blocks_readiness)} disabled={disabled} onBlur={onValidate}
        ...
        <Button disabled={disabled} color="error" onClick={() => onChange({ ...value, validator_mappings: mappings.filter((_, i) => i !== index) })}>Remove mapping {mapping.mapping_id}</Button>
      </Stack>
    })}
