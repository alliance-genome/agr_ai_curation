...
export default function OutputFieldEditor({ format, definition, binding, value, onChange, direct = false, readOnly = false }: Props) {
  ...
  return <Box component="section" sx={{ display: 'grid', gap: 1 }}>
    <Typography sx={{  fontWeight: 600 }}>Choose how to build the file</Typography>
    {!direct && <RadioGroup value={selected ? 'selected_fields' : 'guided'} aria-label="File content mode"
      ...
      <FormControlLabel value="selected_fields" control={<Radio disabled={readOnly} />} label="Use selected fields" />
      <FormControlLabel value="guided" control={<Radio disabled={readOnly} />} label="Let AI arrange the output" />
    </RadioGroup>}
    ...
    <Button variant={selected ? 'outlined' : 'text'} onClick={() => void edit()} disabled={readOnly || sources.length === 0}>Choose output fields</Button>
    <Dialog open={open} maxWidth="lg" fullWidth aria-labelledby="output-fields-title">
      ...
      <DialogContent dividers sx={{ display: 'grid', gap: 2 }}>
        ...
        {!loading && !error && <>
              ...
              return <Fragment key={source.sourceNodeId}>
                ...
                {fields.filter((f) => `${f.label} ${f.group || ''}`.toLowerCase().includes(search.toLowerCase())).map((field) => <TableRow key={`${source.sourceNodeId}:${field.ref}`}>
                  <TableCell><Checkbox checked={columns.some((c) => c.source_node_id === source.sourceNodeId && c.field_ref === field.ref)}
                    slotProps={{ input: { 'aria-label': `Include ${source.sourceLabel}: ${field.label}` } }} onChange={() => toggle(source.sourceNodeId, field.ref, sources.length > 1 ? `${source.sourceLabel} — ${field.label}` : field.label)} /></TableCell>
                  <TableCell>{field.group && <Typography variant="caption" sx={{  display: "block" }}>{field.group}</Typography>}{field.label}</TableCell>
          ...
          <Typography sx={{  fontWeight: 600 }}>Your output · {columns.length} fields</Typography>
          ...
          <Table size="small" aria-label="Selected output fields" sx={{ border: 1, borderColor: 'divider' }}>
            ...
            <TableBody>{columns.length === 0 ? <TableRow><TableCell colSpan={4}>Select fields above to build your output.</TableCell></TableRow> : columns.map((column, index) => <TableRow key={`${column.source_node_id}:${column.field_ref}`}>
              <TableCell><TextField size="small" value={column.header} slotProps={{ htmlInput: { 'aria-label': `Column ${index + 1} name` } }} onChange={(e) => setColumns((current) => current.map((c, i) => i === index ? { ...c, header: e.target.value } : c))} /></TableCell>
