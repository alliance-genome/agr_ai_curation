...
type Props = {
  ...
  onOpenFlow?: (flow: FlowSummaryResponse) => void
}

/** Personal shortcuts only: none of these controls deletes or edits a saved flow. */
export default function FlowShortcutCards({ flows, selectedIds, onChange, saving, onRun, onStop, runningId, canRun, onOpenWorkspace, onOpenFlow }: Props) {
  ...
  return <Stack spacing={1.5}>
    ...
    {selected.length === 0 && <Box sx={{ p: 3, border: 1, borderColor: 'divider', borderRadius: 2, textAlign: 'center' }}>
      <Typography sx={{  fontWeight: 600 }}>Your flow list is empty</Typography>
      ...
      {selected.map((flow, index) => <Box component="li" key={flow.id}
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 0.5, p: 1 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography variant="body2" sx={{ fontWeight: 600, overflowWrap: 'anywhere' }}>{flow.name}</Typography>
            <Typography variant="caption" color="text.secondary">{flow.step_count} steps</Typography>
            {flow.visibility === 'project' && <Typography variant="caption" color="text.secondary" sx={{  display: "block" }}>Shared · Owner #{flow.user_id}</Typography>}
          </Box>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 0.5, ml: 'auto' }}>
          {onOpenFlow && <Button size="small" onClick={() => onOpenFlow(flow)}>{flow.is_owner ? 'Edit in Flow Builder' : 'View in Flow Builder'}</Button>}
    ...
    <Dialog open={pickerOpen} onClose={() => setPickerOpen(false)} fullWidth maxWidth="sm">
      ...
      <DialogContent sx={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        ...
        <Stack role="region" aria-label="Saved flows to add" tabIndex={0} spacing={1} sx={{ overflowY: 'auto', minHeight: 0, maxHeight: '50dvh', pr: 1, overscrollBehavior: 'contain' }}>
          {available.map(flow => <Stack key={flow.id} direction="row" spacing={2} sx={{ alignItems: "center", border: 1, borderColor: 'divider', borderRadius: 1, p: 1.5, flexShrink: 0 }}>
            <Box sx={{ flex: 1, minWidth: 0 }}><Typography variant="body2" sx={{ fontWeight: 600, overflowWrap: 'anywhere' }}>{flow.name}</Typography><Typography variant="body2" color="text.secondary">{flow.description}</Typography></Box>
