...
const DocumentsPage: React.FC = () => {
  ...
  const [paginationModel, setPaginationModel] = useState<DocumentTablePaginationModel>({
    page: 0,
    pageSize: DEFAULT_DOCUMENTS_PAGE_SIZE,
  });
  const [sortModel, setSortModel] = useState<DocumentTableSortModel>([
  ...
  const handlePaginationModelChange = React.useCallback((nextPaginationModel: DocumentTablePaginationModel) => {
    setPaginationModel(nextPaginationModel);
    setSelectedDocumentIds([]);
  }, []);

  const handleSortModelChange = React.useCallback((nextSortModel: DocumentTableSortModel) => {
