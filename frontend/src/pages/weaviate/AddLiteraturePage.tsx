...
AGRKB:101000000055784`}
                  ...
                  <Alert severity="info" sx={{ borderRadius: 1 }}>
                    Resolve is a dry run. Resolve and Import starts PDF-backed jobs that should continue in the background and remain visible from Documents even if you navigate away.
                  </Alert>
                </>
              ) : (
                <Paper
                    ...
                    >
                      <Stack spacing={1.25} alignItems="center">
                        ...
                        <Typography variant="caption" color="text.secondary" sx={{ maxWidth: 560 }}>
                          Upload starts from Add Literature. Processing should continue in the background, with progress and final documents visible from the Documents page.
                        </Typography>
          ...
          <Alert severity="info" icon={<PlayArrowIcon fontSize="inherit" />} sx={{ borderRadius: 1 }}>
            Successful source retrievals and manual PDF uploads both land in the existing Documents table, job panel, chat loader, and download flow.
          </Alert>
        </Box>
        ...
        <Paper variant="outlined" sx={{ borderRadius: 1, overflow: 'hidden' }}>
          <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                PDF-backed source retrievals and upload jobs land in the normal Documents page.
              </Typography>
            </Box>
            <Chip
                  ...
                  return (
                    <TableRow key={`${result.identifier}-${result.status}-${result.documentId ?? result.jobId ?? result.message}`} hover>
                      ...
                      <TableCell align="right">
                        <Tooltip title={canOpen ? 'View in Documents' : result.status === 'resolved' ? 'Ready to import' : 'No PDF-backed document available'}>
                          <span>
                            <IconButton
                              size="small"
                              color="success"
                              ...
                              aria-label={`View ${result.filename ?? result.identifier} in Documents`}
                            >
