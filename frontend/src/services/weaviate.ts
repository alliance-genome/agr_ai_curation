...
export const normalizeDocumentSourceProvenance = (
  raw: RawDocumentSourceProvenance | null
  ...
  const normalized: DocumentSourceProvenance = {
    ...
    providerMetadata: normalizeDocumentSourceProviderMetadata(raw.provider_metadata),
...
export const fetchDocumentDetail = async (id: string): Promise<DocumentDetailData> => {
  const payload = await fetchApi<DocumentResponse>(`/documents/${id}`);
