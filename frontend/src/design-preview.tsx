  ...
  return <ThemeProvider theme={theme}><CssBaseline />
    <StudioPreviewFrame name={value.name} chatOpen={chat} onChatChange={setChat} onNew={() => setSwitchExample('blank')} onExample={() => setSwitchExample('reagents')} dark={dark} onAppearance={() => setDark(!dark)}>
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{  justifyContent: "space-between", alignItems: { sm: 'center' }, gap: 1 }}>
        ...
        <Stack direction="row" sx={{  gap: 1 }}><Button sx={{ textTransform: 'none' }} onClick={() => setSwitchExample('blank')}>Start your own</Button><Button sx={{ textTransform: 'none' }} onClick={() => setSwitchExample('reagents')}>See reagent example</Button></Stack>
      </Stack>
