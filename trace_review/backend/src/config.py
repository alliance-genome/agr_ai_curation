...
def get_agent_studio_trace_search_default_limit() -> int:
    """Return the shared default TraceReview search page request size."""
    return max(1, int(os.getenv("AGENT_STUDIO_TRACE_SEARCH_DEFAULT_LIMIT", "25")))


def get_agent_studio_trace_search_max_limit() -> int:
    """Return the shared maximum TraceReview search page request size."""
    return max(
        get_agent_studio_trace_search_default_limit(),
        int(os.getenv("AGENT_STUDIO_TRACE_SEARCH_MAX_LIMIT", "100")),
    )
