...
for argument in "$@"; do
  case "${argument}" in
    --curator-id-token|--curator-id-token=*)
      ...
      echo "Export ${TOKEN_ENV_NAME}, or set it in the uncommitted smoke env file selected by ${TOKEN_ENV_FILE_NAME}, then rerun the wrapper." >&2
      exit 2
      ;;
  esac
done
...
if [[ "${IS_ADD_LITERATURE_UPLOAD_SMOKE}" == "1" ]]; then
  AWS_PROFILE_NAME="${ADD_LITERATURE_UPLOAD_SMOKE_AWS_PROFILE:-${AWS_PROFILE_NAME:-${AWS_PROFILE:-}}}"
else
  AWS_PROFILE_NAME="${ABC_LITERATURE_READY_UPLOAD_SMOKE_AWS_PROFILE:-${AWS_PROFILE_NAME:-${AWS_PROFILE:-}}}"
fi
...
if [[ -z "${CURATOR_ID_TOKEN_PRESENT}" ]]; then
  if [[ -z "${AWS_PROFILE_NAME}" ]]; then
    echo "An AWS profile is required when no curator ID token is configured." >&2
    echo "Set the smoke-specific AWS profile variable, AWS_PROFILE_NAME, or AWS_PROFILE." >&2
    exit 2
  fi
  mkdir -p "${TMP_AWS_DIR}"
