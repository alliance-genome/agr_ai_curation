...
run_sandbox_helper() {
  ...
  (
    cd "${REPO_ROOT}"
    env \
      -u SYMPHONY_MAIN_SANDBOX_FRONTEND_PORT \
      -u SYMPHONY_MAIN_SANDBOX_BACKEND_PORT \
      -u SYMPHONY_MAIN_SANDBOX_POSTGRES_PORT \
      -u SYMPHONY_MAIN_SANDBOX_REDIS_PORT \
      -u SYMPHONY_MAIN_SANDBOX_LANGFUSE_PORT \
      -u SYMPHONY_MAIN_SANDBOX_WEAVIATE_HTTP_PORT \
      -u SYMPHONY_MAIN_SANDBOX_WEAVIATE_GRPC_PORT \
      -u SYMPHONY_MAIN_SANDBOX_TRACE_REVIEW_FRONTEND_PORT \
      -u SYMPHONY_MAIN_SANDBOX_TRACE_REVIEW_BACKEND_PORT \
      -u SYMPHONY_MAIN_SANDBOX_DB_TUNNEL_LOCAL_PORT \
      -u SYMPHONY_MAIN_SANDBOX_DB_TUNNEL_DOCKER_PORT \
      SYMPHONY_MAIN_SANDBOX_ROOT="${sandbox_root}" \
      bash "${SCRIPT_PATH}" "$@"
  )
}
