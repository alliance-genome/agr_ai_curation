...
Options:
  --project NAME     Incus project that owns the VM (default: default)
...
if [[ -z "${INCUS_PROJECT}" ]]; then
  echo "Could not determine Incus project; pass --project or set SYMPHONY_INCUS_PROJECT." >&2
  exit 2
fi
...
vm_state="$(
  "${incus_cmd[@]}" list "${VM_NAME}" --format csv -c ns 2>/dev/null \
    | awk -F, 'NR == 1 { print $2 }'
)"

if [[ -z "${vm_state}" ]]; then
  echo "Incus VM not found in project ${INCUS_PROJECT}: ${VM_NAME}" >&2
  exit 1
fi

if [[ "${vm_state}" != "RUNNING" ]]; then
  echo "Incus VM is not running in project ${INCUS_PROJECT}: ${VM_NAME} (${vm_state})" >&2
  exit 1
fi

vm_home="$(
  "${incus_cmd[@]}" exec "${VM_NAME}" -- getent passwd "${VM_USER}" 2>/dev/null \
    | cut -d: -f6
)"

if [[ -z "${vm_home}" ]]; then
  echo "Could not resolve home directory for ${VM_USER} inside ${VM_NAME} (project ${INCUS_PROJECT})" >&2
  exit 1
fi

vm_uid="$("${incus_cmd[@]}" exec "${VM_NAME}" -- id -u "${VM_USER}")"
vm_gid="$("${incus_cmd[@]}" exec "${VM_NAME}" -- id -g "${VM_USER}")"
...
if [[ "${FORCE}" -ne 1 && -n "${vm_sha}" && "${vm_sha}" == "${host_sha}" ]]; then
  echo "Codex auth already in sync for ${VM_NAME}:${vm_auth_path} (project ${INCUS_PROJECT})"
  exit 0
fi

if [[ "${DRY_RUN}" -eq 1 ]]; then
  if [[ -n "${vm_sha}" ]]; then
    echo "Would update ${VM_NAME}:${vm_auth_path} (project ${INCUS_PROJECT})"
  else
    echo "Would create ${VM_NAME}:${vm_auth_path} (project ${INCUS_PROJECT})"
  fi
  exit 0
fi
...
if [[ "${new_vm_sha}" != "${host_sha}" ]]; then
  echo "Codex auth sync verification failed for ${VM_NAME}:${vm_auth_path} (project ${INCUS_PROJECT})" >&2
  exit 1
fi

echo "Synced Codex auth to ${VM_NAME}:${vm_auth_path} (project ${INCUS_PROJECT})"
