...
hook_issues=""
upstream=""
upstream_refresh_status="not_needed"
...
if git -C "${workspace_dir}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [[ "${branch}" == "HEAD" || -z "${branch}" ]]; then
    ...
    if [[ -z "${upstream}" ]]; then
      configured_remote="$(git -C "${workspace_dir}" config --get "branch.${branch}.remote" 2>/dev/null || true)"
      configured_merge_ref="$(git -C "${workspace_dir}" config --get "branch.${branch}.merge" 2>/dev/null || true)"

      if [[ -n "${configured_remote}" && "${configured_remote}" != "." && "${configured_merge_ref}" == refs/heads/* ]]; then
        remote_branch="${configured_merge_ref#refs/heads/}"
        remote_tracking_ref="refs/remotes/${configured_remote}/${remote_branch}"
        upstream_refresh_status="failed"

        # Symphony workspaces intentionally clone only main. An issue branch can
        # therefore be configured with an upstream and already pushed while its
        # remote-tracking ref is absent locally. Refresh only that configured ref
        # before deciding whether the branch is synchronized.
        if git -C "${workspace_dir}" fetch --quiet "${configured_remote}" \
          "+${configured_merge_ref}:${remote_tracking_ref}" >/dev/null 2>&1; then
          upstream="${configured_remote}/${remote_branch}"
          upstream_refresh_status="fetched"
        fi
      else
        upstream_refresh_status="not_configured"
      fi
    fi

    if [[ -z "${upstream}" ]]; then
      if [[ "${upstream_refresh_status}" == "failed" ]]; then
...
payload="$(jq -cn \
  ...
  --arg upstream_refresh_status "${upstream_refresh_status}" \
  ...
  --arg state_status "${state_status}" '
  {
    ...
    in_progress_complete_upstream_refresh_status: $upstream_refresh_status,
