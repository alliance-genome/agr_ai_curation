...
run_core_config() {
  local home_dir="$1"
  local input_text="$2"
  printf '\n%s' "$input_text" | HOME="$home_dir" INSTALL_IMAGE_TAG="sha-abcdef1" bash "$core_config_script"
}
...
if [[ "${1:-}" == "run" && " $* " == *" /usr/share/nginx/html/build-metadata.json "* ]]; then
...
EOF
  ...
  assert_contains "Restart command: ${repo_root}/scripts/install/install.sh --from-stage 6" "$output_path"
  assert_not_contains 'Restart command: docker compose' "$output_path"
