...
JSON keeps selected groups as objects and lists as arrays. CSV and TSV show whole groups and lists as readable text in a cell; selecting individual parts gives them separate columns. Ask AI Chat to split list items into numbered or explicitly named columns when needed. Every item must fit; excess items cause an error, not silent truncation. Records from different source steps remain separate rows, with blanks in columns that belong to another source. They are not joined into a new biological record. See [Workshop results and outputs](WORKSHOP_OUTPUT_GUIDE.md) for examples.
