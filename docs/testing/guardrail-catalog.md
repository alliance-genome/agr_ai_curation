...
| G13 | Frontend image runtime configuration | The frontend image renders the shared PDF byte limit and public `VITE_*` browser configuration at startup, while rejecting malformed or non-positive PDF limits before Nginx starts. | ALL-728, ALL-828 | `scripts/testing/frontend_nginx_pdf_limit_contract.sh` |
