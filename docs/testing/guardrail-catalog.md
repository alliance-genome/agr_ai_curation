...
| G3 | Visible terminal formatter binding | Terminal CSV/TSV/JSON flow steps bind a visible formatter agent to completed flow artifacts, and the removed hidden planner/direct-save helpers stay absent. | ALL-601 / wave-74 | `backend/tests/unit/lib/flows/test_tsv_formatter_flow_export.py` |
...
| S8 | ABC Literature Docker smoke token preflight | Docker smoke token mode uses the canonical env-file secret path, skips AWS setup, and rejects an unsupported token CLI form before preflight. | ALL-772 | `scripts/testing/abc_literature_ready_upload_smoke_docker.sh` |
...
| G11 | Backend executable flow topology parity | The authoritative backend projection matches complete language-neutral golden projections and ordered issue codes. | ALL-726 | `backend/tests/unit/lib/flows/test_executable_graph.py` |
| G12 | Frontend executable flow topology parity | The advisory frontend mirror matches the same complete language-neutral golden projections and ordered issue codes. | ALL-726 | `frontend/src/components/AgentStudio/FlowBuilder/executableFlowGraph.test.ts` |
| G13 | Frontend image runtime configuration | The frontend image renders the shared PDF byte limit and deployment-allowlisted public browser configuration at startup, round-trips escaped values, and rejects malformed, missing, or incomplete runtime settings before Nginx starts. | ALL-728, ALL-828 | `scripts/testing/frontend_nginx_pdf_limit_contract.sh` |
