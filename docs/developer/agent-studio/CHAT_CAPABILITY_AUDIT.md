...
`report_tool_failure` uses Sentry only and returns `capture_status` plus `sentry_capture_queued`. The success/error audit status describes local capture acceptance, not ingestion or notification; `notification_submitted` remains false because delivery is not verified. There is no SNS fallback or tool-failure feature flag. Suggestion delivery is separate and retains its SNS `delivery_status`, notification and suggestion references. Neither local capture nor notification-service acceptance proves inbox receipt or developer review.
