...
Execution cadence: implement one slice at a time, validate it, run a GPT-6 Sol
...
The same-host Midscene curator-agent suite in `agent_tests/midscene/` does not alter
these acceptance criteria during its pilot. It runs only against the local
Docker development stack through loopback (including when invoked directly on
the designated dev host), is non-blocking, and neither replaces Chris's manual
...
Per-run verdicts track deduplicated model tokens and a versioned API-cost
estimate; this accounting is informational and is not a provider billing cap.
...
1. Run targeted validation for that slice locally and, when appropriate, on the
   deployed dev stack.
2. Run a GPT-6 Sol xhigh code-review subagent pass focused on the slice's changed
   files and behaviors.
