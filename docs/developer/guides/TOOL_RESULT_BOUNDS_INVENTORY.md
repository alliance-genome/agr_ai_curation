...
The coordinator deferred B2 and B4 through B7 to follow-up tickets on 2026-09-22. Each keeps the disposition recorded below.
...
- **B3. `resolve_domain_field_term`. Closed.** It clamps its candidate limit to `TOOL_PAGE_MAX_LIMIT`, and since extraction no longer searches (ALL-1276), the resolver ledger that needed its complete output is gone, so its byte size is enforced like any run-state result.
