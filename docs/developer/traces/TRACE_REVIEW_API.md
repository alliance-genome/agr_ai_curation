...
traces (default 100) and fetches at most `TRACE_REVIEW_SESSION_MAX_PAGES`
observation pages (default 200). `TRACE_REVIEW_SESSION_TRACE_PAGE_SIZE` controls
the observation page size (default 100); existing observation/request budgets
also apply. Duplicate observations do not consume additional trace slots.
An extra unique trace produces `stop_reason: "trace_limit"`; an unvisited page
at the page cap produces `stop_reason: "page_limit"`. The configured caps are
returned as `session.langfuse_meta.trace_limit` and `page_limit`. Only admitted
...
`stop_reason` is `trace_limit`, `page_limit`, `request_limit`, or `observation_limit`.
