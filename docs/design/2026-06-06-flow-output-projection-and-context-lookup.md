...
There are no conditional or fallback transforms. `first_non_empty` and
`conditional` were removed in the Sep 22 hotfix (ALL-1283): a column maps to one
field, and a resolved value and its paper wording are offered side by side.
...
- Pure unit: column/key rename, omit, reorder, missing value, filter, sort,
  grouping, and derived transforms.
