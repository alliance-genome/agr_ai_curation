...
- [x] Rebuild and verify the release metadata on dev, create an annotated tag
...
- [x] Put the root application into externally verified maintenance mode
  before backups or checkout/deploy changes; keep the independent `/uploads/`
  route explicitly inventoried.
- [x] Create and verify PostgreSQL and native Weaviate backups, recording exact
  backup identifiers and status.
- [x] Deploy only the approved release tag, run migrations, verify backend,
  frontend, Weaviate, TraceReview, Langfuse, Loki, and a server-local PDF route
  while maintenance remains active.
- [x] Restore both nginx routes correctly, pass the public PDF route preflight,
...
- [x] Run a fresh production local-PDF extraction when a safe authorized test
...
- [x] Save the deployment record and curator-facing Slack release-note draft.
...
- 2026-08-23: Merged PR #629 as `60cdc179` and published annotated tag/GitHub
  release `v0.8.21`. The merge commit's Git tree exactly matches approved PR
  head `463db570` (`2c0f3e74f4a2a4f5ec8ab3759a6f46ec1a7daa71`). Dev checked
  out the exact tag, rebuilt the full stack with embedded version `0.8.21` and
  SHA `60cdc179`, remained at migration head `d6e7f8a9b0c1`, and returned all
  services healthy. Backend, frontend, changelog, TraceReview, Langfuse, and
  Loki checks passed; the dev-only Sentry project 3 reports the full tag SHA.
  The tagged Debbie document still has 99 chunks, zero missing pages, pages
  1-21, Figure Legends on page 5, and Funding/References on page 13.
- 2026-08-23: Production entered externally verified maintenance at 17:01 UTC.
  Verified PostgreSQL backup
  `/home/ubuntu/backup_v0821_20260823T170202Z.sql` is 2,178,141,943 bytes with
  complete footer and SHA-256 `8b672a329b25426c175f2a041c181c728e29aacc14add537ee2a4ca0e76105a2`;
  native Weaviate backup `pre-deploy-v0821-20260823-170202` is `SUCCESS` for
  both release collections. The clean production checkout was created from a
  byte-verified full Git bundle after locked production SSH auth was left
  untouched. It runs exact tag `v0.8.21`, commit `60cdc179`, and approved tree
  `2c0f3e74` at migration head `d6e7f8a9b0c1`.
- 2026-08-23: Two deployment-process hazards were found and recovered without
  data loss. The shared Compose project caused the first main-stack `down` to
  remove maintenance briefly; maintenance was immediately restarted before
  continuing. More importantly, the new checkout's relative Weaviate bind
  mounts initially selected empty directories. Public maintenance was
  restored as soon as zero tenants were detected; the new empty directories
  were preserved, and the ignored tagged-checkout paths now symlink to the
  intact original production data and native-backup directories. Both
  collections again expose all 37 tenants, the backup remains `SUCCESS`, and
  the checked tenant retains 35,525 chunks and 343 PDF documents.
- 2026-08-23: Final production cutover completed at 17:26 UTC. Both nginx root
  and `/uploads/` target port 3002, the standalone maintenance container is
  removed, and the banner is `# No maintenance`. Public root is HTTP 200 with
  no maintenance marker; its bundle contains version `0.8.21`, full release
  SHA `60cdc179`, and changelog id `2026-08-23-v0.8.21`. Final public PDF
  evidence is
  `/home/ubuntu/release_evidence/v0.8.21/pdf_viewer_route_preflight_20260823T172628Z.json`
  (`HEAD 200`, range `206`). Backend, TraceReview, Langfuse, Loki, and
  Weaviate all returned 200. A normal curator-default chat returned HTTP 200
  and its temporary session was deleted. No fresh production ingestion is
