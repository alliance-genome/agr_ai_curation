...
| GE-71 | Active validator bindings own final Alliance Gene identity decisions and materialize `primary_external_id`, `gene_symbol`, `taxon`; when unresolved, preserve the evidence-backed candidate in staging hints or finalization ambiguities and leave the unresolved state to validator result fields and envelope validation findings. | `<validator_handoff>` (verbatim "Active validator bindings own final Alliance Gene identity decisions"; the Phase C lean cut de-duplicated the former `<output_and_handoff_contract>` restatement into this single home) |
