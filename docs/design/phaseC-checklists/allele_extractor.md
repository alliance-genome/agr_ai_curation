...
| AE-06 | Retained items are individual allele/variant identities under the active organism-specific group rules, not excluded strains, construct/reagent mentions, balancers, deficiencies, or wild-type references. Active group-specific rules are authoritative and may classify stable inherited transgenic/reporter lines as alleles. | `<success_criteria>` + `<evidence_and_curation_rules>` |
| AE-07 | Each retained allele is supported by direct experimental data in this paper (phenotype, genetic cross, complementation, molecular characterization, rescue, or another result-producing assay) or by an explicit organism-specific group rule defining additional curatable experimental-genotype context. | `<success_criteria>` + `<evidence_and_curation_rules>` |
...
| AE-31 | By default, alleles mentioned only as strain background, controls, methods inventory, or cited from prior work without new data are not retained. An active group-specific rule may explicitly require a stable named allele/line from experimental-genotype or background context and takes precedence; otherwise leave the mention unstaged. | `<evidence_and_curation_rules>` |
...
| AE-33 | Entity mentions in methods/protocol descriptions are not findings by themselves unless an active group-specific rule explicitly treats a stable named line in the experimental genotype/background as curatable; otherwise retain a methods entity only when the paper also presents experimental results involving it. | `<evidence_and_curation_rules>` |
| AE-34 | Distinguish alleles from strains, construct/reagent-only mentions, balancer chromosomes, chromosomal deficiencies/duplications, and wild-type references. Active group-specific rules determine whether stable transgenic, reporter, or driver lines are alleles for that organism. Parse composite genotype strings into individual alleles. | `<evidence_and_curation_rules>` |

## Nomenclature / disambiguation (general, plus active group rules)

| ID | Load-bearing rule | New home |
|----|-------------------|----------|
| AE-35 | Allele naming conventions vary by organism, and active group-specific rules take precedence over the general defaults. Lab allele codes identify the originating lab; HGVS notation is for human/clinical variants; standard wild-type/lab strains are not alleles unless context identifies a specific studied allele; absent a group-specific override, construct/reagent-only transgene mentions and expression drivers are tools rather than alleles; balancers and deficiencies/duplications are genetic tools. | `<evidence_and_curation_rules>` |
| AE-36 | Common false positives (do not stage): strains unless the name identifies a specific allele; transgene constructs/expression drivers used only as tools unless the active group rules classify the stable inherited line as an allele; balancers, deficiencies, chromosomal tools, and wild-type reference states. | `<evidence_and_curation_rules>` |
...
| AE-39 | Weak/non-curatable allele evidence: gene-only statements with no specific allele/variant; strain inventory/stock list/background-genotype text naming no stable allele/line required by the active group rules; prior-work citations or catalog-style mentions copied from earlier papers; generic statements that never identify the tested allele or observed result. | `<evidence_and_curation_rules>` |
