...
| CHV-32 | If no chemical input or expected result field is present, return an unresolved result with empty resolved data, identify the missing input context, and explain that no ChEBI lookup could run because no chemical lookup target was supplied. | `<result_contract>` |
