...
> grounding via grouped `get_ontology_terms` with residual `get_ontology_term`,
> ontologytermtype `CHEBITerm`, NOT `chebi_api_call`;
...
| ECV-12 | Bounded path step 2: group compatible ontology CURIE work across every target in the request and call `get_ontology_terms` once with the combined unique `terms` list before any single-term lookup; assign the shared result and lookup attempt back to each matching component. | `<lookup_workflow>` (ordered step 2) |
| ECV-12A | Bounded path step 3: for components not resolved by the grouped call, use only the owning capability's residual `agr_curation_query` method and record that call in the component's `lookup_attempts`. | `<lookup_workflow>` (ordered step 3; preserves the per-component provenance obligation while avoiding single-term-first execution) |
| ECV-13 | Bounded path step 4: resolve a component only when tool evidence returns a single unambiguous match satisfying its expected fields; preserve ambiguous, obsolete, conflicting, or alternate records as that component's candidates and leave it unresolved. | `<lookup_workflow>` (ordered step 4) |
| ECV-14 | Bounded path step 5: judge relation coherence using `condition_relation_type` as context only — does the resolved condition class fit the stated relation? — without looking up the relation vocabulary. | `<lookup_workflow>` (ordered step 5) |
| ECV-15 | Bounded path step 6: compose the condition-level decision — `status`/`condition_status` resolved only when every required component resolved and the condition-level expected fields can be populated from lookup evidence; otherwise unresolved with the blocking components listed in `unresolved_components`. | `<lookup_workflow>` (ordered step 6) |
