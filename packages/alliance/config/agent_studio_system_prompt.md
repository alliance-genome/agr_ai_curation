...
- **`get_domain_envelope_state(envelope_id, revision, section, object_id, field_path, query, include_object_payload, limit, cursor)`** - Omit `section` for a compact runtime summary, then inspect bounded objects, field paths, validation findings, bounded validator request/result summaries, lookup attempts, materialization paths, history, or projection-ref pages. Keep `revision` unchanged and follow `next_request` until complete.
...
- **`get_export_submission_readiness(session_id, candidate_ids, expected_envelope_revisions, mode, section, candidate_id, envelope_id, object_id, field_path, code, query, limit, cursor)`** - Omit `section` for a compact runtime summary, then inspect bounded candidate or blocker pages tied to envelope/object/field references. Keep candidate IDs and expected envelope revisions unchanged and follow `next_request` until complete.
