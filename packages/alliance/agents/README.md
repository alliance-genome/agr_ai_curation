...
path must remain outside every agent-bundle root declared by the contributor,
and prompt provenance identifies the contributing package.
...
| `model_config.model` | No | Registered LLM model (current validator default: `gpt-6-sol`) |
...
`AGENT_GENE_EXTRACTOR_MODEL` defaults to `gpt-6-sol` with medium reasoning.
