...
def _stage_materializable_observation_with_conditions(
    ledger: resolver_call_ledger.ResolverCallLedger,
):
    """Stage a materializable GE observation carrying two grounded experimental conditions."""
...
def test_gene_expression_builder_materializes_staged_condition_relations(active_builder_context):
    """Staged condition_relations land on the GeneExpressionAnnotation in validator shape."""

    workspace, ledger, _events = active_builder_context
    _stage_materializable_observation_with_conditions(ledger)
    ...
    payload = workspace.finalization.payload
    annotation = payload["curatable_objects"][0]
    assert annotation["object_type"] == "GeneExpressionAnnotation"
...
def test_gene_expression_builder_omits_condition_relations_when_unstaged(active_builder_context):
