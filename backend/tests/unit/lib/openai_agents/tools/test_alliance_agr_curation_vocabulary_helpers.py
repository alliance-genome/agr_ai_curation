    ...
    assert calls == [
        ...
        (
            "go",
            {
                ...
                "exact_match": False,
                "include_synonyms": True,
                # No slim is wired on the cellular_component field (the coarse Public-Site
                # CC display slim regressed ciliary CC extraction), so the GO search keeps
                # the default limit.
                "limit": 25,
            },
        ),
...
def test_domain_field_term_options_routes_gene_expression_site(monkeypatch):
    ...
    assert calls == [
        ...
        (
            "go",
            {
                # No slim is wired on the cellular_component field, so the GO search keeps
                # the caller's limit (the coarse Public-Site CC slim regressed extraction).
                "term": "nucleus",
                "go_aspect": "cellular_component",
                "exact_match": False,
                "include_synonyms": True,
...
# =============================================================================
# Subset-aware ontology lookups (Part B: data-type-axis slims for ONTOLOGY search).
# =============================================================================


def _ontology_term(curie, name, ontology_type="GOTerm", namespace="cellular_component"):
    return SimpleNamespace(
        curie=curie,
        name=name,
...
class _FakeSlimSession:
    """Fake SQLAlchemy session resolving vocabularytermset member CURIEs by slim name.

    Mirrors the real ``_ontology_slim_member_curies`` query which selects UPPER(vt.name).
    """

    def __init__(self, members_by_slim):
        self._members_by_slim = {
            key.upper(): list(values) for key, values in members_by_slim.items()
        }
        self.executed = []
        self.closed = False

    def execute(self, _query, params):
        slim_name = params.get("slim_name")
        self.executed.append(slim_name)
        members = self._members_by_slim.get(str(slim_name).upper(), [])
        return SimpleNamespace(
            fetchall=lambda: [(str(curie).upper(),) for curie in members]
        )
...
class _SlimDb:
    """Fake curation DB exposing ontology search helpers + a slim-member session."""

    def __init__(self, *, ontology_terms, go_terms, members_by_slim, raise_session=False):
        self._ontology_terms = ontology_terms
        self._go_terms = go_terms
        self._members_by_slim = members_by_slim
        self._raise_session = raise_session
        self.session = None
        self.last_ontology_limit = None
        self.last_go_limit = None

    def search_ontology_terms(self, **kwargs):
        self.last_ontology_limit = kwargs.get("limit")
        return list(self._ontology_terms)

    def search_go_terms(self, **kwargs):
        self.last_go_limit = kwargs.get("limit")
        return list(self._go_terms)

    def create_session(self):
        if self._raise_session:
            raise RuntimeError("transient db error")
        self.session = _FakeSlimSession(self._members_by_slim)
        return self.session


_CC_SLIM_MEMBERS = {
    "Cellular Components Slim Terms Public Site": [
        "GO:0005575",
        "GO:0005634",
        "GO:0005829",
        "otherLocations",  # non-CURIE placeholder member; never matches a real curie
    ],
}

# Search returns a mix of in-slim and out-of-slim GO cellular-component terms.
_CC_GO_RESULTS = [
    _ontology_term("GO:0005634", "nucleus"),
    _ontology_term("GO:0005829", "cytosol"),
    _ontology_term("GO:0099080", "supramolecular complex"),  # NOT a slim member
    _ontology_term("GO:0070013", "intracellular organelle lumen"),  # NOT a slim member
]


def _slim_resolver(monkeypatch, **kwargs):
    db = _SlimDb(
        ontology_terms=kwargs.pop("ontology_terms", _CC_GO_RESULTS),
        go_terms=kwargs.pop("go_terms", _CC_GO_RESULTS),
        members_by_slim=kwargs.pop("members_by_slim", _CC_SLIM_MEMBERS),
        raise_session=kwargs.pop("raise_session", False),
    )
    monkeypatch.setattr(
        agr_curation,
        "get_curation_resolver",
...
def test_search_go_terms_without_slim_returns_full_ontology(monkeypatch):
    _slim_resolver(monkeypatch)
    result = _query_fn()(
    ...
    assert result.status == "ok"
    curies = {item["curie"] for item in result.data}
    # No slim -> full result set, including out-of-slim terms.
    assert curies == {"GO:0005634", "GO:0005829", "GO:0099080", "GO:0070013"}
    assert not any(
        str(w).startswith("ontology_slim_applied") for w in (result.warnings or [])
    )


def test_search_go_terms_with_cc_slim_restricts_to_members(monkeypatch):
    _slim_resolver(monkeypatch)
    result = _query_fn()(
        method="search_go_terms",
        term="nucleus",
        go_aspect="cellular_component",
        slim="Cellular Components Slim Terms Public Site",
        limit=20,
    )
    assert result.status == "ok"
    curies = {item["curie"] for item in result.data}
    # Only slim-member CURIEs survive; out-of-slim GO terms are dropped.
    assert curies == {"GO:0005634", "GO:0005829"}
    assert (
        "ontology_slim_applied:Cellular Components Slim Terms Public Site"
        in (result.warnings or [])
    )


def test_search_go_terms_with_slim_overfetches_then_reapplies_user_limit(monkeypatch):
    db = _slim_resolver(monkeypatch)
    result = _query_fn()(
        method="search_go_terms",
        term="nucleus",
        go_aspect="cellular_component",
    ...
    assert result.status == "ok"
    # The DB search ran with the large effective limit (query-level over the bounded
    # slim, not a post-filter of a tiny relevance-truncated page).
    assert db.last_go_limit == 500
    # The user's limit=1 is re-applied AFTER restriction.
    assert len(result.data) == 1
    assert result.data[0]["curie"] in {"GO:0005634", "GO:0005829"}


def test_search_ontology_terms_with_slim_restricts_to_members(monkeypatch):
    _slim_resolver(monkeypatch)
    result = _query_fn()(
        method="search_ontology_terms",
        term="nucleus",
...
def test_ontology_slim_unknown_name_yields_empty(monkeypatch):
    _slim_resolver(monkeypatch)
    result = _query_fn()(
        method="search_go_terms",
        term="nucleus",
        go_aspect="cellular_component",
        slim="Nonexistent Slim",
        limit=20,
    )
    assert result.status == "ok"
    # An unknown slim is a real curated answer of zero CURIE members -> restrict to none.
    assert result.count == 0
    assert "slim_not_found_or_empty:Nonexistent Slim" in (result.warnings or [])


def test_ontology_slim_fails_open_on_db_error(monkeypatch):
    # create_session raises -> slim resolution fails OPEN -> full unrestricted ontology.
    _slim_resolver(monkeypatch, raise_session=True)
    result = _query_fn()(
        method="search_go_terms",
        term="nucleus",
        go_aspect="cellular_component",
    ...
    assert result.status == "ok"
    curies = {item["curie"] for item in result.data}
    # Fail-open: every candidate is preserved (extraction is NOT regressed).
    assert curies == {"GO:0005634", "GO:0005829", "GO:0099080", "GO:0070013"}
    assert "slim_restriction_unavailable:setup_error" in (result.warnings or [])


def test_ontology_slim_helper_normalizes_and_unions(monkeypatch):
    # Direct helper test: list of slims -> UNION of member CURIEs (case-insensitive).
    db = _SlimDb(
        ontology_terms=[],
        go_terms=[],
        members_by_slim={
            "Slim A": ["GO:0000001"],
            "Slim B": ["GO:0000002", "go:0000003"],
        },
    )
    members, warnings = agr_curation._ontology_slim_member_curies(
        db, ["Slim A", "Slim B"]
    )
    assert members == frozenset({"GO:0000001", "GO:0000002", "GO:0000003"})
    assert warnings == []
    # Absent slim -> no restriction (None), unchanged behavior.
    none_members, none_warnings = agr_curation._ontology_slim_member_curies(db, [])
    assert none_members is None
