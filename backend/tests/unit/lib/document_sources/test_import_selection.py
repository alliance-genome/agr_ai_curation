...
@pytest.mark.asyncio
async def test_select_checksum_import_candidate_routes_abc_tei_only_main_to_conversion():
    source = _source("source-1", provider="abc_literature")
...
@pytest.mark.asyncio
async def test_select_checksum_import_candidate_accepts_adapter_derived_readiness():
    source = _source("source-1", provider="abc_literature")
    markdown = _converted("md-1", "source-1", provider="abc_literature")
    provider = FakeConversionProvider(
        [source],
