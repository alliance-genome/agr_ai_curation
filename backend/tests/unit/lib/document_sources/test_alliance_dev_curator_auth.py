...
def _credentials(
    *, token: str = "id-token", expires_at: float = 5000
) -> auth.DevCuratorCredentials:
...
def test_load_settings_requires_supported_mode_and_all_values(monkeypatch) -> None:
    monkeypatch.setenv("DOCUMENT_SOURCE_DEV_CURATOR_AUTH_MODE", "none")
    with pytest.raises(auth.DevCuratorCredentialUnavailable) as exc_info:
        auth._load_settings()
    assert "username" not in str(exc_info.value).lower()

    monkeypatch.setenv("DOCUMENT_SOURCE_DEV_CURATOR_AUTH_MODE", "bad-mode")
    with pytest.raises(auth.DevCuratorCredentialUnavailable, match="misconfigured"):
        auth._load_settings()

    monkeypatch.setenv(
        "DOCUMENT_SOURCE_DEV_CURATOR_AUTH_MODE", auth.COGNITO_USER_PASSWORD_MODE
    )
    for name in (
...
def test_authenticate_uses_password_flow_and_validates_paired_tokens(
    monkeypatch,
) -> None:
    observed = {}
...
def test_authenticate_suppresses_aws_sdk_debug_secret_logging(
    monkeypatch, caplog
) -> None:
...
def test_authenticate_rejects_challenge_and_malformed_responses(
...
async def test_static_provider_bearer_cannot_substitute_for_dev_curator_mode(
    ...
    with pytest.raises(auth.DevCuratorCredentialUnavailable):
        await auth.resolve_development_credentials()


@pytest.mark.asyncio
async def test_async_resolver_returns_validated_credentials(monkeypatch):
    expected = _credentials()
    monkeypatch.setattr(auth, "_load_settings", _settings)
    monkeypatch.setattr(auth, "_authenticate_sync", lambda settings: expected)
    assert await auth.resolve_development_credentials() is expected
