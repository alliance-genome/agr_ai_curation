...
def test_load_agent_definitions_raises_clear_error_for_missing_package_agent_yaml(tmp_path):
    ...
    _write_package_manifest(
        package_dir,
        {
            ...
            "agent_bundles": [{"name": "demo_agent"}],
        },
    )
    (package_dir / "agents" / "demo_agent").mkdir(parents=True)

    with pytest.raises(
        FileNotFoundError,
        match="Package 'demo.core' agent bundle 'demo_agent' is missing agent.yaml",
    ):
...
def test_resolve_agent_sources_warns_when_package_agent_dir_is_missing_from_agent_bundles(
    ...
    for agent_name in ("demo_agent", "missing_manifest"):
    ...
    assert [source.folder_name for source in sources] == ["demo_agent"]
    ...
    assert [source.folder_name for source in repeated_sources] == ["demo_agent"]
    assert "Ignoring undeclared agent bundle directories for package 'demo.core'" in caplog.text
    assert "agent_bundles is missing package-owned agent directories with agent.yaml" in caplog.text
    assert "agents/missing_manifest" in caplog.text
    ...
    _write_package_manifest(
        package_dir,
        {
            ...
            "exports": [
                {
                    "kind": "prompt",
                    "name": "demo_agent.system",
                    "path": "agents/demo_agent/prompt.yaml",
                    "description": "Base prompt without an owning agent export",
                }
            ],
        },
    )
    prompt_file = package_dir / "agents" / "demo_agent" / "prompt.yaml"
...
def test_resolve_agent_config_sources_allows_config_override_layer(tmp_path):
    ...
    (package_dir / "agents" / "demo_agent").mkdir(parents=True)
    (package_dir / "agents" / "demo_agent" / "agent.yaml").write_text(
        "agent_id: demo_agent_validation\nname: Package Demo Agent\n",
        encoding="utf-8",
    )
    (package_dir / "agents" / "demo_agent" / "prompt.yaml").write_text(
        "content: Package prompt\n",
        encoding="utf-8",
    )

    (overrides_dir / "demo_agent").mkdir(parents=True)
    (overrides_dir / "demo_agent" / "agent.yaml").write_text(
        "agent_id: demo_agent_validation\nname: Override Demo Agent\n",
        encoding="utf-8",
    )
    (overrides_dir / "demo_agent" / "prompt.yaml").write_text(
    ...
    assert {source.folder_name for source in sources} == {"custom_local", "demo_agent"}
    demo_source = next(source for source in sources if source.folder_name == "demo_agent")
    assert demo_source.package_id == "demo.core"
    assert demo_source.agent_dir == (overrides_dir / "demo_agent")
    assert demo_source.agent_yaml == (overrides_dir / "demo_agent" / "agent.yaml")
...
def test_resolve_agent_config_sources_merges_partial_override_without_dropping_package_bundle(tmp_path):
    ...
    _write_package_manifest(
        package_dir,
        {
            ...
            "python_package_root": "python/src/demo_core",
            "requirements_file": "requirements/runtime.txt",
            "agent_bundles": [{"name": "demo_agent", "group_rules": ["demo", "team"]}],
        },
    )
    package_agent_dir = package_dir / "agents" / "demo_agent"
    ...
    (package_agent_dir / "group_rules" / "team.yaml").write_text(
        "content: Package TEAM rules\n",
        encoding="utf-8",
    )
...
def test_merge_group_rule_files_prefers_later_paths():
    ...
    assert merged == (demo_package, team_override)
...
def test_resolve_agent_config_sources_rejects_duplicate_bundles_within_packages_root(tmp_path):
    ...
    with pytest.raises(ValueError, match=r"Duplicate agent bundle 'demo_agent' discovered in "):
        agent_sources.resolve_agent_config_sources(packages_dir)
...
def test_env_override_allows_legacy_agent_directory_loading(tmp_path, monkeypatch):
    ...
    (demo_agent_dir / "agent.yaml").write_text(
        "\n".join(
            [
                "agent_id: demo_agent_validation",
                "name: Demo Validation Agent",
                "output_schema: DemoValidationEnvelope",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    (demo_agent_dir / "prompt.yaml").write_text(
        "agent_id: demo_agent_validation\ncontent: Legacy demo prompt\n",
        encoding="utf-8",
    )
    (group_rules_dir / "demo.yaml").write_text(
        "group_id: DEMO\ncontent: Legacy demo rules\n",
        encoding="utf-8",
    )
    (demo_agent_dir / "schema.py").write_text(
        "\n".join(
            [
    ...
    assert "DemoValidationEnvelope" in schemas
    assert any(
        call["source_file"].endswith("legacy-agents/demo_agent/prompt.yaml")
        for call in captured_calls
    )
