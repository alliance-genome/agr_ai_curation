...
async def test_invalid_regex_through_workshop_can_retry_literal(
    ...
    monkeypatch.setattr(agent_studio, "report_runtime_exception", report)
...
async def test_operational_errors_keep_reporting(repository, monkeypatch, failure):
    report = Mock()
    # Main reports unexpected handler failures through the sanitized runtime
    # reporter (ALL-947); the hotfix's once-helper is not on this path.
    monkeypatch.setattr(agent_studio, "report_runtime_exception", report)
    if failure == "missing":
    ...
    assert report.call_args.kwargs["operation"] == "tool_handler_failed"
