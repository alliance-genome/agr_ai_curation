...
def test_custom_validator_uses_frozen_benchmark_route_without_changing_pin(monkeypatch):
    from src.lib.agent_studio import custom_profile_validators as service, catalog_service
    ...
    pin = {'agent_key': receipt.agent_key, 'revision_id': str(receipt.agent_revision_id), 'fingerprint': receipt.fingerprint}
    context = SimpleNamespace(user_id='request-sub', document_id='paper', authenticated_groups=('GROUP_A',))
    monkeypatch.setattr(service, 'runtime_validator_user_id', lambda _: 7)
    agent = SimpleNamespace(execution_receipt={'fingerprint': pin['fingerprint']})
    build = MagicMock(return_value=agent)
    monkeypatch.setattr(catalog_service, 'get_benchmark_agent_by_id', build)
    slot = 'validator:custom-pinned-validation'
    with benchmark_source_revisions({slot: receipt}), benchmark_route_plan({slot: {'provider': 'openrouter', 'model': 'test-model', 'reasoning_effort': None}}):
        assert service.build_custom_validator_agent(pin, context, benchmark_slot=slot) is agent
        with pytest.raises(ValueError, match="differs"):
            service.build_custom_validator_agent({**pin, "revision_id": str(uuid4())}, context, benchmark_slot=slot)
    build.assert_called_once_with(
        'ca_saved', db_user_id=7,
        user_id='request-sub', document_id='paper', authenticated_groups=['GROUP_A'],
        benchmark_slot=slot,
    )
