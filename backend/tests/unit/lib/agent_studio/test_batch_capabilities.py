...
class TestBatchCapabilities:
    ...
    def test_pdf_agent_has_pdf_extraction_flag(self):
        ...
        agent = AGENT_REGISTRY.get("pdf_extraction")
    ...
    def test_gene_expression_has_pdf_extraction_flag(self):
        ...
        agent = AGENT_REGISTRY.get("gene_expression_extraction")
        assert agent is not None
        assert "batch_capabilities" in agent
        assert "pdf_extraction" in agent["batch_capabilities"]
    ...
    def test_disease_extractor_has_pdf_extraction_flag(self):
        """Disease extractor should have batch_pdf_extraction capability."""
        agent = AGENT_REGISTRY.get("disease_extractor")
        assert agent is not None
        assert "batch_capabilities" in agent
        assert "pdf_extraction" in agent["batch_capabilities"]

    def test_chemical_extractor_has_pdf_extraction_flag(self):
        """Chemical extractor should have batch_pdf_extraction capability."""
        agent = AGENT_REGISTRY.get("chemical_extractor")
        assert agent is not None
        assert "batch_capabilities" in agent
        assert "pdf_extraction" in agent["batch_capabilities"]
    ...
    def test_chat_output_has_chat_output_flag(self):
        ...
        agent = AGENT_REGISTRY.get("chat_output_formatter")
        assert agent is not None
        assert "batch_capabilities" in agent
