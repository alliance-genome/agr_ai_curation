...
async def test_create_flow_accepts_inherited_custom_agent_validation_attachments(
    ...
    monkeypatch.setattr(execution_revisions, "get_execution_revision", lambda *args, **kwargs: (
        ...
        SimpleNamespace(tool_ids=[], group_tool_policy={}, output_contract=pin.output_contract,
                        template_source=None, default_export_execution_mode=None,
                        structured_finalization=None, model_id="catalog-model",
                        model_reasoning="medium", curation={
                            "adapter_key": "gene", "domain_pack_id": "gene", "launchable": True,
                        }),
    ))
    # The pinned revision's model is in the catalog.
    monkeypatch.setattr(execution_revisions, "get_model", lambda model_id: SimpleNamespace(
        supports_reasoning=True, reasoning_options=("low", "medium", "high", "xhigh")))
