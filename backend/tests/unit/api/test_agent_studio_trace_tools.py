...
async def test_failure_report_waits_for_delivery_and_keeps_session(monkeypatch, delivered):
    ...
    assert result["notification_submitted"] is False
    assert result["sentry_capture_queued"] is delivered
    assert result["success"] is delivered
    assert api_module._tool_result_status(result) == ("success" if delivered else "error")
    assert result["status"] == ("success" if delivered else "error")
    assert result["capture_status"] == ("queued" if delivered else "unavailable")
    if not delivered:
        assert "not queued" in result["message"]
        assert "not queued" in api_module._tool_result_error(result)
