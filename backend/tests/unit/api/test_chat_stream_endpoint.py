...
def test_chat_stream_endpoint_reattaches_to_active_same_turn_without_reclaiming(monkeypatch):
    ...
    _patch_chat_impl(
        ...
        lambda _active_groups=None: (_ for _ in ()).throw(AssertionError("reattach should not resolve runner config")),
    )
    keepalive_calls = []
    _patch_chat_impl(
        monkeypatch,
...
def test_chat_stream_endpoint_does_not_repersist_inline_extraction_result(monkeypatch):
    ...
    _patch_chat_impl(monkeypatch, "get_groups_from_provider_groups", lambda _groups: [])
    _patch_chat_impl(monkeypatch, "get_supervisor_tool_agent_map", lambda _active_groups=None: {"ask_gene_expression_specialist": "gene-expression"})
    ...
    async def _run_agent_streamed(**_kwargs):
        yield {"type": "RUN_STARTED", "data": {"trace_id": "trace-inline"}}
        yield {
            "type": chat_common.INTERNAL_EXTRACTION_RESULT_EVENT_TYPE,
            "details": {
                "toolName": "ask_gene_expression_specialist",
                "extraction_result_id": "11111111-1111-4111-8111-111111111111",
                "result_ref": "extraction-result:11111111-1111-4111-8111-111111111111",
            },
            "internal": {
                "canonical_payload": {
                    "envelope_id": "env-1",
    ...
    response = asyncio.run(
        chat.chat_stream_endpoint(
            chat_message=chat.ChatMessage(message="Extract findings", session_id="session-inline"),
            user={"sub": "auth-sub", "cognito:groups": []},
        )
    )
    ...
    assert len(refs) == 1
    assert refs[0].result_ref == "extraction-result:11111111-1111-4111-8111-111111111111"
...
def test_chat_stream_endpoint_uses_runner_emitted_evidence_summary(monkeypatch):
    ...
    _patch_chat_impl(monkeypatch, "document_state", SimpleNamespace(get_document=lambda _uid: None))
    _patch_chat_impl(monkeypatch, "get_groups_from_provider_groups", lambda _groups: [])
    _patch_chat_impl(monkeypatch, "get_supervisor_tool_agent_map", lambda _active_groups=None: {"ask_gene_extractor_specialist": "gene_extractor"})
    _patch_chat_impl(
        monkeypatch,
...
def test_chat_stream_endpoint_infers_scope_for_scope_free_extraction_envelopes(monkeypatch):
    ...
    _patch_chat_impl(
        monkeypatch,
        "get_supervisor_tool_agent_map",
        lambda _active_groups=None: {"ask_gene_extractor_specialist": "gene_extractor"},
    )
    monkeypatch.setattr(
        extraction_results_module,
        "_get_agent_curation_metadata",
