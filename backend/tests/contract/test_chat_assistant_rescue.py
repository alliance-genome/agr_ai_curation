...
@pytest.mark.parametrize(
    ("retry_payload", "expected_detail"),
    [
        (
            {
                "content": "rescued conflicting reply",
                "trace_id": "trace-owned-1",
            },
            "Assistant rescue payload conflicts with existing assistant turn for fields: content",
        ),
        (
            {
                "content": "rescued durable reply",
                "trace_id": "trace-owned-2",
            },
            "Assistant rescue payload conflicts with existing assistant turn for fields: trace_id",
        ),
    ],
)
def test_assistant_rescue_rejects_mismatched_retry_payload(
    contract_client,
    chat_contract_auth_headers,
    chat_contract_db,
    seed_chat_contract_session,
    ...
    session_id = f"contract-rescue-mismatch-{uuid4().hex[:8]}"
    turn_id = f"turn-owned-{uuid4().hex[:8]}"
    seed_chat_contract_session(
        session_id=session_id,
        title="Owned rescue session",
    ...
    conflict_response = contract_client.post(
        f"/api/chat/{session_id}/assistant-rescue",
        headers=chat_contract_auth_headers,
        json={
            "turn_id": turn_id,
    ...
    assert conflict_response.json()["detail"] == expected_detail
