...
def test_stage_phenotype_tool_requires_rationale_with_shared_description():
    ...
    from agr_ai_curation_alliance.tools.phenotype_builder_tools import stage_phenotype_observation
...
def test_phenotype_annotation_declares_optional_rationale_in_evidence_group():
    registry = load_alliance_domain_pack_registry()
    pack = registry.get_pack(PHENOTYPE_DOMAIN_PACK_ID)
    assert pack is not None
    definition = next(
