"""
Chemical Ontology Agent using OpenAI Agents SDK.

This agent specializes in querying the ChEBI database
for chemical entity information, classifications, and ontology relationships.
"""
...
from ..models import ChemicalResult, ChemicalResultEnvelope
...
def create_chemical_agent() -> Agent:
    """
    Create a Chemical Ontology specialist agent.
    ...
    Returns:
        An Agent instance configured for Chemical Ontology queries
    """
    ...
    config = get_agent_config("chemical")
    log_agent_config("Chemical Specialist", config)

    # Create a restricted REST API tool for ChEBI only
    chebi_api_tool = create_rest_api_tool(
    ...
    base_prompt = get_prompt("chemical")
    ...
    instructions = inject_structured_output_instruction(
        instructions,
        output_type=ChemicalResultEnvelope
    )
    ...
    log_agent_config(
        agent_name="Chemical Specialist",
