"""JSON Formatter Agent using OpenAI Agents SDK.

This agent formats data as JSON files for download.
...
from ..tools.file_output_tools import create_json_tool

logger = logging.getLogger(__name__)


def create_json_formatter_agent() -> Agent:
    """Create a JSON Formatter agent.
    ...
    Returns:
        An Agent instance configured for JSON formatting
    """
    # Get config from registry + environment
    config = get_agent_config("json_formatter")
    log_agent_config("JSON Formatter", config)

    # Get prompts from cache (zero DB queries at runtime)
    base_prompt = get_prompt("json_formatter", "system")
