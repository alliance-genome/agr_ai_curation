...
class SupervisorRouting:
    ...
    def tool_name(self) -> str:
        """Generated at runtime from folder name - not stored here."""
        raise NotImplementedError("tool_name is set externally based on folder name")
...
class AgentDefinition:
    ...
    def from_yaml(cls, folder_name: str, data: Dict[str, Any]) -> "AgentDefinition":
        ...
        if model_str.startswith("${") and ":-" in model_str:
            # Extract default value: ${AGENT_GENE_MODEL:-gpt-4o} -> gpt-4o
            default_val = model_str.split(":-")[1].rstrip("}")
            model_str = default_val
