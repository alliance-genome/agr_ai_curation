...
def reserve_runtime_request(measurement: dict) -> RuntimeAccountingAttempt | None:
    ...
    with SessionLocal() as db:
        ...
        db.add(RuntimeCostRequest(
            ...
            document_id=context.document_id, job_id=context.job_id,
            invocation_id=context.invocation_id, parent_invocation_id=context.parent_invocation_id,
            operation_type=measurement.get("operation_type", "model"),
            candidate_count=measurement.get("candidate_count"), pagination_request=measurement.get("pagination_request"),
