...
2. Reconstruct exact `task_instructions`, every present `custom_instructions`, and each judgment-relevant `step_goal` with `get_current_flow_instructions(node_id, field, cursor, limit)`. Follow `next_cursor` until `complete=true` for every required field.
3. Inspect the `get_current_flow_topology` sections `issues`, `control_path`, `control_edges`, `output_bindings`, and `validation_sidecars`. Fetch relevant `get_current_flow_node` scalar details, `get_current_flow_projection_plan` field or JSON-Pointer sections, `get_current_flow_validation_warnings` pages, and `get_current_flow_validation_schedule` sections (`selections`, `scheduled_validators`, `opt_outs`, `replacement_validators`, `supplemental_validators`, `inactive_metadata`) only when the verification criteria require them. Follow every cursor/completion signal.
...
6. For document/PDF capability claims, use `get_tool_inventory(agent_id=<node agent>)` or another focused query and method/PDF-level `get_tool_details(tool_id, agent_id)`, never the unsafe global inventory or oversized parent-tool metadata.
