...
def build_custom_validator_agent(pin, runtime_context, *, benchmark_slot: str | None = None):
    """Reauthorize the exact custom revision again at dispatch, including tools."""
    from src.lib.agent_studio.catalog_service import get_agent_by_id, get_benchmark_agent_by_id
    from src.lib.openai_agents.benchmark_routing import benchmark_route_plan_active
    ...
    benchmark = benchmark_slot is not None and benchmark_route_plan_active()
    if benchmark:
        from src.lib.benchmarks.source_revisions import require_benchmark_source
        source = require_benchmark_source(benchmark_slot, pin["agent_key"])
        if str(source.agent_revision_id) != pin["revision_id"] or source.fingerprint != pin["fingerprint"]:
            raise ValueError("Custom validator differs from its frozen benchmark source")
    construction = get_benchmark_agent_by_id if benchmark else get_agent_by_id
    agent = construction(
        pin["agent_key"], db_user_id=user_id,
        ...
        **({"benchmark_slot": benchmark_slot} if benchmark else {"execution_revision_id": pin["revision_id"]}),
    )
