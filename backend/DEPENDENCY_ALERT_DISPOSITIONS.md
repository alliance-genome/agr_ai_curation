...
- Current dependency path: `agr-curation-api-client==0.10.1` -> `fastapi-okta==1.4.0` and `agr-cognito-py==0.1.0` -> `python-jose==3.5.0` -> `ecdsa==0.19.2`.
- Why it is still present: this repository imports `agr_curation_api.db_methods.DatabaseMethods` in `backend/src/lib/database/curation_resolver.py`, and the latest published `agr-curation-api-client` release on PyPI still has those auth dependencies bundled unconditionally.
