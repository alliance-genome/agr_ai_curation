"""Upsert phenotype system agent into unified agents table.
...
def _load_phenotype_spec() -> Tuple[Dict[str, Any], str, str] | None:
    source = next(
        (
            item
    ...
    if agent_yaml is None or prompt_yaml is None:
        raise RuntimeError(f"Missing phenotype agent bundle for '{_AGENT_KEY}'.")
    ...
    spec = {
        ...
        "name": str(agent_data.get("name") or "Phenotype Extraction Agent"),
    ...
    connection.execute(
        sa.text(
            ...
            VALUES (
                ...
                true,
                :created_by,
                'Seeded phenotype prompt for unified agents runtime parity',
                :source_file
            )
            """
        ),
...
def upgrade() -> None:
    loaded_spec = _load_phenotype_spec()
